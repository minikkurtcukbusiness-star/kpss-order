...
const MAX_SORU_PAKET=5,MAX_TEST=40,MOCK_EXAM_SORU=20,MAX_JSON_DENEME=2;
function modelListesi(){return aiProvider.getModels();}
function temizJson(metin){if(!metin||typeof metin!=="string")return null;let x=metin.trim().replace(/^```json\s*/i,"").replace(/^```\s*/i,"").replace(/\s*```$/i,"");try{return JSON.parse(x)}catch{}const baslar=[x.indexOf("{"),x.indexOf("[")].filter(i=>i>=0);if(!baslar.length)return null;const bas=Math.min(...baslar),ac=x[bas],kap=ac==="{"?"}":"]";let d=0,str=false,esc=false;for(let i=bas;i<x.length;i++){const c=x[i];if(esc){esc=false;continue}if(c==="\\"&&str){esc=true;continue}if(c==='"'){str=!str;continue}if(str)continue;if(c===ac)d++;if(c===kap&&--d===0){try{return JSON.parse(x.slice(bas,i+1))}catch{return null}}}return null;}
function soruGecerliMi(s){const sec=s?.secenekler;if(!s||typeof s.soru!=="string"||!s.soru.trim()||!sec||typeof sec!=="object")return false;return ["A","B","C","D","E"].every(h=>typeof sec[h]==="string"&&sec[h].trim())&&["A","B","C","D","E"].includes(String(s.dogruCevap||"").toUpperCase());}
function promptOlustur(konular,toplam,oncekiler=[]){const konuMetni=konular.map((k,i)=>`${i+1}. Ders: ${k.subject}\nKonu: ${k.topic}\nZorluk: ${k.difficulty}\nSoru: ${k.count}`).join("\n\n");const onceki=oncekiler.length?`\nBenzer üretme; önceki soruların başlangıçları:\n${oncekiler.slice(-10).map(x=>`- ${x}`).join("\n")}`:"";return `Türkiye KPSS Ortaöğretim için uzman öğretmen olarak TAM OLARAK ${toplam} özgün çoktan seçmeli soru üret.\n${konuMetni}${onceki}\nKurallar: A-E beş seçenek, tek doğru cevap, açık Türkçe, gerçekçi çeldiriciler, kısa öğretici açıklama. JSON dışında hiçbir şey yazma.\nSADECE şu JSON'u döndür: {"sorular":[{"subject":"Türkçe","topic":"...","soru":"...","secenekler":{"A":"...","B":"...","C":"...","D":"...","E":"..."},"dogruCevap":"A","aciklama":"..."}]}`;}
async function paketUret(istekler,toplam,oncekiler=[]){let last=null;for(let deneme=1;deneme<=MAX_JSON_DENEME;deneme++){try{console.log(`[OpenRouter] ${toplam} soru isteniyor. JSON denemesi ${deneme}/${MAX_JSON_DENEME}`);const content=await aiProvider.generate({system:"Sen yalnızca geçerli JSON üreten KPSS soru yazarı ve öğretmensin.",prompt:promptOlustur(istekler,toplam,oncekiler),jsonMode:true,maxTokens:3000});const parsed=temizJson(content);const raw=Array.isArray(parsed)?parsed:(Array.isArray(parsed?.sorular)?parsed.sorular:[]);const valid=raw.filter(soruGecerliMi).slice(0,toplam).map((s,i)=>({id:`ai-${Date.now()}-${i}-${Math.random().toString(36).slice(2,7)}`,subject:s.subject||istekler[0]?.subject||"KPSS",topic:s.topic||istekler[0]?.topic||"Genel",soru:s.soru.trim(),secenekler:{A:String(s.secenekler.A),B:String(s.secenekler.B),C:String(s.secenekler.C),D:String(s.secenekler.D),E:String(s.secenekler.E)},dogruCevap:String(s.dogruCevap).toUpperCase(),aciklama:String(s.aciklama||"")}));if(valid.length<toplam)throw new Error(`Model ${toplam} yerine ${valid.length} geçerli soru döndürdü.`);return valid;}catch(e){last=e;console.error(`[OpenRouter] Paket denemesi ${deneme} başarısız: ${e.message}`)}}throw last||new Error("Geçerli soru üretilemedi.");}
